report 50406 "Sales Pro Forma Invoice"
{
    // version NAVW113.00,NAVDACH13.00,NAVCH13.00

    DefaultLayout = RDLC;
    RDLCLayout = 'Sales Pro Forma Invoice.rdl';
                    ...
                    if SalesLine.FindSet() then
                        repeat
                            if SalesLine.Type = SalesLine.Type::Item then begin
                                ...
                                clear(txtSerialInfos);
                                recTrackingLine.reset;
                                recTrackingLine.SetFilter("Source Type", '%1', 37);
                                recTrackingLine.SetFilter("Source Subtype", '%1', "Sales Header"."Document Type");
                                recTrackingLine.SetFilter("Source ID", '%1', SalesLine."Document No.");
                                recTrackingLine.SetFilter("Source Ref. No.", '%1', SalesLine."Line No.");
                                if recTrackingLine.FindSet() then begin
                                    inttab := 1;
                                    txtSerialInfos[inttab] := txtSerialInfosLbl + ' ';
                                    repeat
                                        if strlen(txtSerialInfos[inttab] + recTrackingLine."Serial No." + ', ') > 50 then begin
                                            inttab += 1;
                                            txtSerialInfos[inttab] := recTrackingLine."Serial No." + ', ';
                                        end
                                        else
                                            txtSerialInfos[inttab] += recTrackingLine."Serial No." + ', '
                                    until recTrackingLine.Next() = 0;
                                end;
    ...
    var
        Text004: Label 'Sales Pro Forma Invoice %1', Comment = '%1 = Document No.';
        ...
        OrderNoCaptionLbl: Label 'Sales Pro Forma Invoice No.';
