report 50211 TransferShipmentNew //base on sales invoice proforma
{
    // version NAVW113.00,NAVDACH13.00,NAVCH13.00

    DefaultLayout = RDLC;
    RDLCLayout = 'Transfer Shipment.rdl';
    Caption = 'Transfer Shipment'; //base on 'Sales Pro Forma Invoice';
    ...
    dataset
    {
        dataitem("Transfer Shipment Header"; "Transfer Shipment Header")
        {
            ...
            begin
                ...
                SalesLine.DELETEALL;
                daPostDate := "Transfer Shipment Header"."Posting Date";
                intNewLineNo := 10000;
                codTranRcpNo := "Transfer Shipment Header"."No.";
                recTransferShipmentLine.SetFilter("Document No.", '%1', "Transfer Shipment Header"."No.");
                recTransferShipmentLine.SetFilter(SalesOrderNo, '<>%1', '');
                recTransferShipmentLine.Findset();
                begin
                    "Sales Header".SetFilter("Document Type", '%1', "Sales Header"."Document Type"::Order);
                    "Sales Header".SetFilter("No.", '%1', recTransferShipmentLine.SalesOrderNo);
                    "Sales Header".FindFirst();
                    codOrderNo := recTransferShipmentLine.SalesOrderNo;
                    repeat
                        recSalesLine1.get("Sales Header"."Document Type"::Order, recTransferShipmentLine.SalesOrderNo, recTransferShipmentLine.SalesOrderLineNo);
                        SalesLine.TransferFields(recSalesLine1);
                        SalesLine."Purchase Order No." := recTransferShipmentLine."Document No.";//recSalesLine1."Document No.";
                        salesline."Purch. Order Line No." := recTransferShipmentLine."Line No.";//recSalesLine1."Line No.";
                        SalesLine."Line No." := intNewLineNo;
                        SalesLine.Quantity := recTransferShipmentLine.Quantity;
                        SalesLine."Quantity (Base)" := recTransferShipmentLine."Quantity (Base)";
                ...
                if SalesLine.FindSet() then
                    repeat
                        if SalesLine.Type = SalesLine.Type::Item then begin
                            ...
                            RetrieveEntriesFromShptRcpt(recTempItemLedgEntry, DATABASE::"Transfer Shipment Line", 0, SalesLine."Purchase Order No.", '', 0, salesLine."Purch. Order Line No.");
        ...
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(DocDate_SalesHeader; format(daPostDate, 0, '<Day,2>.<Month,2>.<Year4>'))//format("Sales Header"."Posting Date", 0, '<Day,2>.<Month,2>.<Year4>'))//Format("Sales Header"."Document Date", 0, '<Day,2>.<Month,2>.<Year4>'))//FORMAT("Sales Header"."Document Date",0,'<Day,2>.<Month,2>.<Year4>'))
                    {
                    }
    ...
    var
        Text004: Label 'Return of Consignment %1', Comment = '%1 = Document No.';
        ...
        OrderNoCaptionLbl: Label 'Return of Consignment No.';
    ...
    begin
        // retrieves a data set of Item Ledger Entries (Posted Shipments/Shipments)
