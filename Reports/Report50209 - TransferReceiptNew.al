report 50209 TransferReceiptNew //base on sales invoice proforma
{
    // version NAVW113.00,NAVDACH13.00,NAVCH13.00

    DefaultLayout = RDLC;
    RDLCLayout = 'Transfer Receipt.rdl';
    Caption = 'Transfer Receipt'; //base on 'Sales Pro Forma Invoice';
    ...
    dataset
    {
        dataitem("Transfer Receipt Header"; "Transfer Receipt Header")
        {
            ...
            var
                ...
                intNewLineNo: Integer;
                intSourceLineNo: Integer;
            ...
            begin
                ...
                SalesLine.DELETEALL;
                intNewLineNo := 10000;
                codTranRcpNo := "Transfer Receipt Header"."No.";
                recTransferReceiptLine.SetFilter("Document No.", '%1', "Transfer Receipt Header"."No.");
                recTransferReceiptLine.SetFilter(SalesOrderNo, '<>%1', '');
                recTransferReceiptLine.Findset();
                begin
                    "Sales Header".SetFilter("Document Type", '%1', "Sales Header"."Document Type"::Order);
                    "Sales Header".SetFilter("No.", '%1', recTransferReceiptLine.SalesOrderNo);
                    "Sales Header".FindFirst();
                    codOrderNo := recTransferReceiptLine.SalesOrderNo;
                    repeat
                        recSalesLine1.get("Sales Header"."Document Type"::Order, recTransferReceiptLine.SalesOrderNo, recTransferReceiptLine.SalesOrderLineNo);
                        SalesLine.TransferFields(recSalesLine1);
                        SalesLine."Purchase Order No." := recTransferReceiptLine."Document No.";//recSalesLine1."Document No.";
                        salesline."Purch. Order Line No." := recTransferReceiptLine."Line No.";//recSalesLine1."Line No.";
                        SalesLine."Line No." := intNewLineNo;
                        SalesLine.Quantity := recTransferReceiptLine.Quantity;
                        SalesLine."Quantity (Base)" := recTransferReceiptLine."Quantity (Base)";
                        SalesLine."Document No." := codOrderNo;
                        SalesLine.SuspendStatusCheck(true);
                        SalesLine.validate("Line Discount %");
                        SalesLine.Insert(false);
                        SalesLine2.TransferFields(SalesLine);
                        SalesLine2.Insert(false);
                        intSourceLineNo := intNewLineNo;
                        intNewLineNo += 10000;
                        recSalesLine2.SetFilter("Document Type", '%1', recSalesLine1."Document Type");
                        recSalesLine2.SetFilter("Document No.", '%1', recSalesLine1."Document No.");
                        recSalesLine2.SetFilter("Attached to Line No.", '%1', recSalesLine1."Line No.");
                        if recSalesLine2.FindSet() then
                            repeat
                                SalesLine.TransferFields(recSalesLine2);
                                SalesLine."Purchase Order No." := recSalesLine2."Document No.";
                                salesline."Purch. Order Line No." := recSalesLine2."Line No.";
                                SalesLine."Document No." := codOrderNo;
                                SalesLine."Line No." := intNewLineNo;
                                SalesLine."Attached to Line No." := intSourceLineNo;
                                SalesLine.Insert(false);
                                SalesLine2.TransferFields(SalesLine);
                                SalesLine2.Insert(false);
                                intNewLineNo += 10000;
                ...
                //if Confirm('A ' + format(SalesLine.Count())) then;
                        ...
                        if SalesLine.Type = SalesLine.Type::Item then begin
                            if CompressArray(txtSerialInfos) > 0 then begin
                                for inttab := 1 to CompressArray(txtSerialInfos) do begin
                                    ...
                                    recSalesLineTMP."Document Type" := SalesLine."Document Type";
                            ...
                            intlineNoSrc := SalesLine."Line No.";
                            clear(txtSerialInfos);
                            recTempItemLedgEntry.Deleteall();
                            RetrieveEntriesFromShptRcpt(recTempItemLedgEntry, DATABASE::"Transfer Receipt Line", 0, SalesLine."Purchase Order No.", '', 0, salesLine."Purch. Order Line No.");
                ...
                if CompressArray(txtSerialInfos) > 0 then begin
                    for inttab := 1 to CompressArray(txtSerialInfos) do begin
                        recSalesLineTMP.init();
                        recSalesLineTMP."Document Type" := "Sales Header"."Document Type";
                        recSalesLineTMP."Document No." := "Sales Header"."No.";
                        recSalesLineTMP."Line No." := intlineNoSrc + (inttab * 100);
                        recSalesLineTMP.Insert(false);
                        recSalesLineTMP.Type := recSalesLineTMP.Type::" ";
                    ...
                    if recSalesLineTMP.FindSet() then
                        repeat
                            SalesLine.TransferFields(recSalesLineTMP);
                            SalesLine.Insert(false);
                            SalesLine2.TransferFields(SalesLine);
                            SalesLine2.Insert(false);
                ...
                if SalesLine.FindSet() then;
                //if Confirm(format(SalesLine.Count())) then;
                CurrReport.Break();
        ...
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(No_SalesHeader; codTranRcpNo) //"No.")
            {
            }
            ...
            column(intCommentCount; intCommentCount)
            {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(DocDate_SalesHeader; format("Transfer Receipt Header"."Posting Date", 0, '<Day,2>.<Month,2>.<Year4>'))// Format("Sales Header"."Document Date", 0, '<Day,2>.<Month,2>.<Year4>'))//FORMAT("Sales Header"."Document Date",0,'<Day,2>.<Month,2>.<Year4>'))
                    {
                    }
                    ...
                    dataitem("InventoryCommentLine"; "Inventory Comment Line")
                    {
                        ...
                        trigger OnPreDataItem()
                        begin
                            SetFilter("Document Type", '%1', InventoryCommentLine."Document Type"::"Posted Transfer Receipt");
                            SetFilter("No.", '%1', codTranRcpNo);
                            if not FindSet then
                                CurrReport.Skip();
                        end;
                            ...
                            */
                            if SalesLine.type = salesline.type::item then begin
                                if SalesLine.Quantity <> 0 then
                                    SalesLine."Unit Price" := SalesLine."Line Amount" / SalesLine.Quantity;
                                txtDesc2 := SalesLine.ExtRef;
                                xintAttacheLineNo := -1;
                                SalesLine2.setfilter("Document Type", '%1', SalesLine."Document Type");
                                SalesLine2.SetFilter("Document No.", '%1', SalesLine."Document No.");
                                SalesLine2.SetFilter("Attached to Line No.", '%1', SalesLine."Line No.");
            ...
            begin
                ...
                END;*/
                recInvCommentLine.SetFilter("Document Type", '%1', recInvCommentLine."Document Type"::"Posted Transfer Receipt");
                recInvCommentLine.SetFilter("No.", '%1', codTranRcpNo);
                if recInvCommentLine.FindSet then
                    intCommentCount := recInvCommentLine.Count();
            end;
            ...
            begin
                ...
                InnerGroupPageNO := 1;
                "Sales Header".SetFilter("Document Type", '%1', "Sales Header"."Document Type"::Order);
                "Sales Header".SetFilter("No.", '%1', codOrderNo);
    ...
    var
        Text004: Label 'Consignation %1', Comment = '%1 = Document No.';
        ...
        OrderNoCaptionLbl: Label 'Consignation No.';
        ...
        ML_OrderAdr: Label 'Invoice Address';
        ...
        codOrderNo: code[20];
        codTranRcpNo: code[20];
        intCommentCount: Integer;
